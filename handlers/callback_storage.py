...
async def handle_storage_callbacks(client, callback_query, user_id: int, data: str, s: dict):
    ...
    if data == "set_personal_bot_token":
        ...
        await callback_query.message.reply_text("ðŸ¤– Ab BotFather wala bot token bhejo.\nExample: `123456:ABCDEF...`\n\n/cancel bhej kar cancel kar sakte ho.")
        await callback_query.answer()
        return True
    ...
    if data == "set_gdrive_token_file":
        ...
        await callback_query.message.reply_text("â˜ï¸ Ab token.pickle file bhejo.\n\n/cancel bhej kar cancel kar sakte ho.")
        await callback_query.answer()
        return True
    if data == "set_gdrive_folder_id":
        set_user_state(user_id, "set_gdrive_folder_id")
        await callback_query.message.reply_text("â˜ï¸ Ab Google Drive folder ID bhejo.\n\n/cancel bhej kar cancel kar sakte ho.")
        await callback_query.answer()
        return True
    ...
    if data == "validate_gdrive_settings":
        try:
            ...
            await callback_query.answer(f"âœ… {result.get('name','ok')}", show_alert=True)
        except Exception as e:
            await callback_query.answer(f"âŒ {e}", show_alert=True)
        return True
    if data == "set_rclone_config_file":
        set_user_state(user_id, "set_rclone_config_file")
        await callback_query.message.reply_text("ðŸ—‚ Ab rclone.conf file bhejo.\n\n/cancel bhej kar cancel kar sakte ho.")
        await callback_query.answer()
        return True
    if data == "set_rclone_remote_path":
        set_user_state(user_id, "set_rclone_remote_path")
        await callback_query.message.reply_text("ðŸ—‚ Ab remote path bhejo. Example: myremote:Telegram/Folder\n\n/cancel bhej kar cancel kar sakte ho.")
        await callback_query.answer()
        return True
    ...
    if data == "validate_rclone_settings":
        try:
            ...
            await callback_query.answer(f"âœ… {result.get('path','ok')}", show_alert=True)
        except Exception as e:
            await callback_query.answer(f"âŒ {e}", show_alert=True)
        return True
    ...
    if data == "validate_personal_bot":
        ...
        try:
            ...
            await callback_query.answer(f"âœ… @{getattr(me, 'username', 'unknown')}", show_alert=True)
        except Exception as e:
            await callback_query.answer(f"âŒ {e}", show_alert=True)
        return True
